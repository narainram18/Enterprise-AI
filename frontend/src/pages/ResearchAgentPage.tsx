...
export function ResearchAgentPage() {
  ...
  const [selectedAgentId] = useState('research-assistant')
  ...
  async function runAssistantStream(
    ...
    const callbacks: StreamCallbacks = {
      ...
      onToolProgress: (toolName) => {
        ...
        else if (toolName === 'web_search') status = 'Searching the web...'
        else if (toolName === 'browser_tool') status = 'Reading webpage...'
        ...
        <div className="conversation-heading">
          {isRenaming ? <div className="conversation-rename"><input value={renameTitle} onChange={(event) => setRenameTitle(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void saveConversationRename(); if (event.key === 'Escape') setIsRenaming(false) }} aria-label="Conversation title" autoFocus /><IconButton label="Save conversation title" onClick={() => void saveConversationRename()}><Save size={16} /></IconButton><IconButton label="Cancel rename" onClick={() => setIsRenaming(false)}><X size={16} /></IconButton></div> : <>
            <div className="research-header-title">
              <h2>{title}</h2>
              <span className="research-badge"><SparklesIcon size={12}/> Research Mode</span>
            </div>
          </>}
        </div>
        <div className="research-filters">
          <button className="research-filter-btn"><FileText size={14}/> Specific document</button>
          <button className="research-filter-btn"><Search size={14}/> Document type</button>
        </div>
        ...
        {isLoadingConversation ? <LoadingState label="Loading conversation…" /> : loadError ? <ErrorState message={loadError} onRetry={() => routeConversationId && navigate(`/app/chat/${routeConversationId}`)} /> : messages.length ? <div ref={messageListRef} className="message-list" onScroll={onMessageListScroll} aria-live="polite">{messages.map((item) => (
          <motion.article initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={item.id} className={`chat-message chat-message-${item.role.toLowerCase()} ${item.isStreaming || item.isThinking ? 'is-streaming' : ''}`}>
            ...
            <div className="message-content">
              ...
              {item.role === 'ASSISTANT' ? item.isThinking ? <div className="thinking-state"><LoaderCircle className="spin" size={14} />{item.toolStatus || 'Searching knowledge base…'}</div> : (
                <>
                  ...
                  {item.citations && item.citations.length > 0 && (
                    <div className="research-sources-container">
                      <div className="research-sources-header">
                        <p className="citations-heading">Sources cited</p>
                        {item.retrievalStatistics && (
                          <span className={`confidence-badge ${item.retrievalStatistics.retrievedChunks > 3 ? 'high' : 'medium'}`}>
                            <Check size={12} /> {item.retrievalStatistics.retrievedChunks > 3 ? 'High' : 'Medium'} Confidence
                          </span>
                        )}
                      </div>
                      <div className="source-chips">
                        {item.citations.map((cit, idx) => (
                          <div key={idx} className="source-chip" title={`Score: ${Math.round(cit.similarityScore * 100)}%`}>
                            <FileText size={12} />
                            <span className="source-chip-name">{cit.fileName}</span>
                            <span className="source-chip-page">{cit.pageNumber != null ? `p. ${cit.pageNumber}` : `c. ${cit.chunkIndex}`}</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <div className="research-welcome-wrapper">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="research-welcome-panel">
              <div className="research-welcome-icon"><Search size={32} /></div>
              <h2>Research Assistant</h2>
              <p>Search across your organization's knowledge base. Upload documents and ask questions with cited answers.</p>
              
              <div className="chat-suggested-prompts">
                {[
                  "Summarize all uploaded documents",
                  "Compare two policies",
                  "Find employee leave policy",
              ...
              <div className="research-welcome-actions">
                <Button onClick={() => void sendMessage("Search company handbook")}><SparklesIcon size={16}/> Start Research</Button>
                <button onClick={() => navigate('/app/documents')} className="upload-link"><FileUp size={16} /> Upload Document</button>
              </div>
            </motion.div>
          </div>
        )}
